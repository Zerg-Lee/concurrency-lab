...
channel_t* channel_create(size_t size)
{
    ...
    channel->close_state = 0;
    pthread_mutex_init(&channel->lock, NULL);
    pthread_mutex_init(&channel->innerlock, NULL);
    pthread_cond_init(&channel->fullwait, NULL);
    pthread_cond_init(&channel->emptywait, NULL);
    return channel;
}
...
enum channel_status channel_send(channel_t *channel, void* data)
{
    ...
    {
        pthread_cond_wait(&channel->fullwait, &channel->lock);
    }
    if (channel->close_state == 0)
    {
        pthread_mutex_lock(&channel->innerlock);
        buffer_add(channel->buffer, data);
        pthread_cond_signal(&channel->emptywait);
        pthread_mutex_unlock(&channel->innerlock);
    }
    else
    {
        return CLOSED_ERROR;
...
enum channel_status channel_receive(channel_t* channel, void** data)
{
    ...
    {
        pthread_cond_wait(&channel->emptywait, &channel->lock);
    }
    if (channel->close_state == 0)
    {
        pthread_mutex_lock(&channel->innerlock);
        buffer_remove(channel->buffer,data);
        pthread_cond_signal(&channel->fullwait);
        pthread_mutex_unlock(&channel->innerlock);
    }
    else
    {
...
enum channel_status channel_close(channel_t* channel)
{
    /* IMPLEMENT THIS */
    pthread_mutex_lock(&channel->lock);
    if (channel->close_state==1)
    {
        return CLOSED_ERROR;
    }
    else
    {
        channel->close_state = 1;
    }
    pthread_mutex_unlock(&channel->lock);
    return SUCCESS;
}
...
enum channel_status channel_destroy(channel_t* channel)
{
    ...
    else
    {
        ...
        pthread_cond_destroy(&channel->fullwait);
        pthread_cond_destroy(&channel->emptywait);
